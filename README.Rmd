...
Phydon is super user friendly. With an internal data, a minimal example is as simple as:
...
-   `accession_no`: the accession number of the genome that users want to predict the growth rate.

-   `Opt_temp`: **Optional**. This is used for gRodon predictions the growth temperature of the genome. If not provided, the growth temperature of the genome will be set to 20 degree Celsius.
...
-   `genome`: the accession number of the genome that users want to predict the growth rate
...
> [!Note]\
> The last genome `RS_GCF_102749895.1` was made up on purpose to show the case when the genome is not in the GTDB database. In this case, the representative genome and species will be NA, and the phylogenetic distance will be Inf. The gRodon predictions will be used for this genome.

# Before you start, annotating genomes

> [!IMPORTANT]\
> As gRodon requires annotated genomes, users need to annotate the genomes before using Phydon. We recommend using [Prokka](https://github.com/tseemann/prokka) for genome annotation. Thus, the directory structure of the genomic data should be like this:
...
2.  Organize the genomic data in the `genefiles/` directory and generate `data_info.csv` file.
