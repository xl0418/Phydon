...
#' @param regression_mode The regression model to be used. It can be "arithmetic_mean" or "geometric_mean" that use a continuous P. Or "arithmetic_mean_binaryP" or "geometric_mean_binaryP" use a binary P. Default is "geometric_mean".
...
Phydon <- function(data_info_df, user_tree = NULL, tax = "bacteria", regression_mode="geometric_mean",gRodon_mode="full") {
  ...
  for (i in 1:nrow(data_info_df)) {
    ...
    Est_gRodon <- gRodonpred(gene_location, temp = temp, gRodon_mode = gRodon_mode)
